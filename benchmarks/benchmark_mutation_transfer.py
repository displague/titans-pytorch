...
def parse_args():
    ...
    parser.add_argument("--tag", type = str, default = "mutation_transfer")
...
def build_variant_specs(dim, pages, hier_pages):
    phase_quorum = dict(
        gated = True,
        diag = True,
        gate_mode = "soft",
    ...
    mutation_champion = dict(
        ...
        quorum_temperature = 0.15,
        budget_topk_ratio = 0.3
    )
    ...
    return {
        "symplectic_paging": dict(
            description = "Symplectic paging default",
            model_kwargs = dict(
        ...
        "phase_quorum_paging": dict(
            description = "Phase + quorum (hand-designed)",
            model_kwargs = dict(
                use_symplectic_gating = True,
                num_pages = pages,
        ...
        "quorum_budget_paging": dict(
            description = "Phase + quorum budget (hand-designed)",
            model_kwargs = dict(
                use_symplectic_gating = True,
                num_pages = pages,
                symplectic_gate_kwargs = quorum_budget
            )
        ),
        "hierarchical_paging": dict(
            description = "Hierarchical routing (hand-designed)",
            model_kwargs = dict(
                use_symplectic_gating = True,
                symplectic_gate_kwargs = phase_quorum,
                **hierarchical_kwargs
        ...
        "mutation_champion": dict(
            description = "Mutation/selection champion",
            model_kwargs = dict(
                use_symplectic_gating = True,
                num_pages = pages,
                symplectic_gate_kwargs = mutation_champion
            )
        )
    }
...
if __name__ == "__main__":
    ...
    variants = build_variant_specs(args.dim, args.pages, args.hier_pages)
    ...
    champion = results["mutation_champion"]
    comparisons = {}
    for name in ("symplectic_paging", "phase_quorum_paging", "quorum_budget_paging", "hierarchical_paging"):
        baseline = results[name]
        comparisons[name] = dict(
            long_clean_mse_post_pct = pct_improvement(baseline["long_horizon"]["clean_mse_post"], champion["long_horizon"]["clean_mse_post"]),
            long_phase_err_post_pct = pct_improvement(baseline["long_horizon"]["phase_err_post"], champion["long_horizon"]["phase_err_post"]),
            interference_post_a_pct = pct_improvement(baseline["interference"]["post_a_loss"], champion["interference"]["post_a_loss"]),
            transfer_score_pct = pct_improvement(baseline["transfer_score"], champion["transfer_score"])
        )
    ...
    metrics = dict(
        ...
        winner = winner_name,
        mutation_vs_manual = comparisons
    )

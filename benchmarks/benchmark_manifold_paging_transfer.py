...
def parse_args():
    ...
    parser.add_argument("--tag", type = str, default = "manifold_paging_transfer")
...
def build_variant_specs(dim, pages, threshold):
    gate_kwargs = dict(
        gated = True,
        diag = True,
        gate_mode = "soft",
    ...
    return {
        "threshold_paging": dict(
            description = "Threshold-triggered paging",
            model_kwargs = dict(
        ...
        "manifold_keyed_paging": dict(
            description = "Manifold-state keyed paging",
            model_kwargs = dict(
                use_symplectic_gating = True,
                num_pages = pages,
...
if __name__ == "__main__":
    ...
    variants = build_variant_specs(args.dim, args.pages, args.threshold)
    ...
    threshold = results["threshold_paging"]
    manifold = results["manifold_keyed_paging"]
    comparison = dict(
        long_clean_mse_post_pct = pct_improvement(threshold["long_horizon"]["clean_mse_post"], manifold["long_horizon"]["clean_mse_post"]),
        long_phase_err_post_pct = pct_improvement(threshold["long_horizon"]["phase_err_post"], manifold["long_horizon"]["phase_err_post"]),
        interference_post_a_pct = pct_improvement(threshold["interference"]["post_a_loss"], manifold["interference"]["post_a_loss"]),
        transfer_score_pct = pct_improvement(threshold["transfer_score"], manifold["transfer_score"])
    )
